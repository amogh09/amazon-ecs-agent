...
import (
	...
	tmds "github.com/aws/amazon-ecs-agent/ecs-agent/tmds/handlers/response"
...
type ContainerResponse struct {
	...
	Ports      []tmds.PortResponse   `json:"Ports,omitempty"`
	Networks   []tmds.Network        `json:"Networks,omitempty"`
	Volumes    []tmds.VolumeResponse `json:"Volumes,omitempty"`
}
...
func NewPortBindingsResponse(dockerContainer *apicontainer.DockerContainer, eni *apieni.ENI) []tmds.PortResponse {
	container := dockerContainer.Container
	resp := []tmds.PortResponse{}
...
func NewVolumesResponse(dockerContainer *apicontainer.DockerContainer) []tmds.VolumeResponse {
	container := dockerContainer.Container
	var resp []tmds.VolumeResponse

	volumes := container.GetVolumes()

	for _, volume := range volumes {
		volResp := tmds.VolumeResponse{
