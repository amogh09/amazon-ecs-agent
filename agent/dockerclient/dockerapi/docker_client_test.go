...
func TestManifestFetchTime(t *testing.T) {
	...
	var newClient = func() DockerClient {
		...
		return defaultClient.WithVersion(dockerclient.Version_1_30)
	}
	...
	var newPull = func(image string) result {
		...
		require.NoError(t, err)
		manifestPullTime := time.Now().Sub(start)
	...
	for _, result := range results {
		...
		fmt.Printf("image: %v, old: %v, new: %v, manifest: %v, new_total: %v, impact: %v\n",
			result.image, result.oldResult.pullTime, result.newResult.pullTime,
			result.newResult.manifestFetchTime,
			newTotal,
			newTotal-result.oldResult.pullTime,
		)
	}
}
...
func TestDistributionInspect(t *testing.T) {
	t.Run("dockerhub private", func(t *testing.T) {
		...
		encodedAuthConfig := ""
		require.NoError(t, err)
		res, err := client.DistributionInspect(context.Background(), "ubuntu:latest", encodedAuthConfig)
		require.NoError(t, err)
